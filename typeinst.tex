...
EasyCrypt trabaja con enfoque basado en juego, cuyos principales actores son los ADVERSARIOS y los ORÁCULOS. Este contiene variables globales y una colecciones de procesos: Procesos definidos (Oráculos) y Procesos Abstractos (Adversarsios).
...
Las respuestas que brinda Easycrypt son proof sketches que incrementan la confianza en las mismas produciendo pruebas verificables independientes y también permite generar estas pruebas en formato COQ, para que luego, el usuario si lo desea las ejecute en dicho formato con dicho programa.\cite{article4}



\section{Comparación de Programas.}
\centerline{\emph{CertiCrypt}}
En la parte programacional, EasyCrypt no posee bucles ni procesos recursivos, CertiCrypt si. En el caso de las evidencias generadas, EasyCrypt es parcial por no tener generadores de pruebas COQ (se esta buscando agregar esta característica). Este también genera pruebas en base a la probabilidad y no máquinas totalmente probadas.
...
En ambos programas la eficiencia de cómputos no puede ser expresada formalmente porque sus locas identifican términos hasta computos, por ejemplo, ($\lambda$x. x+x) no puede ser distinguido de el valor 2.\cite{article6}
...
\begin{itemize}
	\item Seguridad ECBC: Se define como una noción basada en juego, mediante el juego de colisión de nombre, en donde el adversario $\mathcal{A}$ se le provee una lista de mensajes a su elección, antes de que la función hash f sea seleccionada de H. Este gana si contiene dos mensajes distintos con la misma imagen de h. Se dice que una función hash f es resistente a colisión si la probabilidad es baja. Formalizando, la resistencia a la colisión de CBC-MAC es una familia de funciones hash indexada por un bloque de clave de cifrado. Esto produce un límite concreto en la seguridad de ECBC.
...
\end{itemize}
El caso fue exitoso ya que probó que CMAC es computacionalmente es indistinguible de FCBC, así también los lemas utilizado para la prueba.\cite{article5}
...
Easycrypt es un programa que para su objetivo es muy eficiente ya que las verficaciones que brinda son mucho mas sencillas. Las actualizaciones que esperan sumarle, agrandarian la calidad y espectro de las respuestas que brinda, como por ejemplo agregarle asistentes como COQ. En comparación a los otros programas y con la gran necesidad de seguridad en estos dias, EasyCrypt es un excelente programa con mucho potencial de mejorar.
...
\begin{itemize}
	...
	\item AT, la respuesta que brinda la correji, fijate que te parece
	\item en seguridad ECBC hay una oracion en ingles qeu nose como traducirla y en XCBC tambien
	\item En cramer puse definiciones y teoremas en ingles, no quiero traducirlos porque seguro algo mal hago nose que queres hacer al respecto
	\item los casos de estudios sin contar CMAC los otros estan, depende cual elijamos lo agrando pero deberian estar mas o menos bien
